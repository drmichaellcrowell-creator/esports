...
**CAS semantics and post-write consistency (Amendment 002, Decision 3).** The following rules were empirically established during Layer 0B and are binding for authority-sensitive concurrency:

- **CAS winner is `updateMany(..., expected version ...)` with `updated === 1`.** A single-record version-guarded write where exactly one record is updated is the winner. **`updated === 0` is stale/conflict** — the state moved since it was read; the operation must fail closed or retry with a fresh read.
- **Post-write reads may be eventually consistent.** A filter immediately after a write may not reflect that write. Correctness-critical post-write confirmation uses a **bounded consistency re-read** (e.g., 3 retries at 250ms = 750ms max). **Exhaustion fails closed** — never assume success.
- **Three distinct retry mechanisms** must not be conflated: (1) consistency retry (bounded re-read after a write), (2) CAS version-guarding (application-owned version increment on a conditional write; a failed guard is a conflict, not a retry), (3) rate-limit retry (linear backoff for transient 429 errors; separate budget).
- **Acceptance trial count.** Acceptance race suites may be executed in batches to stay within Base44 execution limits, but the required aggregate is **25 post-fix trials per required race** for authority-sensitive concurrency gates, unless a later architecture decision explicitly changes the standard.
...
| R14 | Incomplete roster move | Source RosterAssignment recovery metadata says `roster.move` with non-null request `R` and server-computed material-input hash `H`, source is terminal `removed`, but no destination row with `creation_request_id=R` exists after settling window | **No** | **Always.** Destination intent is operator-reviewed; AuditLog is not required for detection. | Hourly |
